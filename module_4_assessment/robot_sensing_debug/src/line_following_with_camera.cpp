...
private:
  void cameraCallback(const sensor_msgs::msg::Image::SharedPtr cameraMsg)
  {
    ...
    int row = 800, column = 0;
    cv::Mat roi = cannyImage(cv::Range(row, row + 280), cv::Range(column, column + 1920));
    ...
    if (!edge.empty())
    {
      ...
      RCLCPP_INFO(this->get_logger(), "Image Width: %f, Image Height: %f", velocityMsg.linear.x, velocityMsg.angular.z);
